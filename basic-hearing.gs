...
function handleUnified(payload) {
    ...
    if (csvContent.trim().replace(/^\uFEFF/, '') !== '') {
        ...
        const withBom = '\uFEFF' + csvContent.replace(/^\uFEFF/, '');
        const blob = Utilities.newBlob(withBom, 'text/csv; charset=utf-8', csvFileName);
        const file = folder.createFile(blob);
...
const withBom = '\uFEFF' + csvContent.replace(/^\uFEFF/, '');
deleteIfExists(folder, fileName);
const blob = Utilities.newBlob(withBom, 'text/csv; charset=utf-8', fileName);
const file = folder.createFile(blob);
